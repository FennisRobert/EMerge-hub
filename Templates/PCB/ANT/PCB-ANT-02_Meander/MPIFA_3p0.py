...
# For routing we'll use the PCB class

pcbd = em.geo.PCB(thpcb, 1.0, em.GCS.displace(x0, y0, 0), trace_material=em.lib.COPPER)

router = pcbd.new(0,0, wfeed, (0,1), 1).straight(ydist-want).turn(90).straight(xpos_feed+wfeed/2).straight(1*mm)

for _ in range(N_meander):
    router = router.straight(lmeander).pturn(90).straight(wmeander).pturn(-90).straight(lmeander).pturn(-90).straight(wmeander).pturn(90)


ant_poly = pcbd.compile_paths(True)
...
model.mesher.set_boundary_size(ant_poly, 1 * mm)
